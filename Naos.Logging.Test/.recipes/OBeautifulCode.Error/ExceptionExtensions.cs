...
namespace OBeautifulCode.Error.Recipes
{
        ...
        static class ExceptionExtensions
    {
        ...
        /// <see cref="Exception.Data"/> dictionary, keyed on <see cref="Constants.ExceptionDataKeyForErrorCode"/>
        ...
        /// dictionary, keyed on <see cref="Constants.ExceptionDataKeyForErrorCodesVector"/>.
        /// </returns>
        ...
        /// <exception cref="ArgumentException"><paramref name="exception"/> <see cref="Exception.Data"/> already contains the key <see cref="Constants.ExceptionDataKeyForErrorCode"/>.</exception>
        /// <exception cref="ArgumentException"><paramref name="exception"/> <see cref="Exception.Data"/> already contains the key <see cref="Constants.ExceptionDataKeyForErrorCodesVector"/>.</exception>
        public static Exception AddErrorCode(
            this Exception exception,
            string errorCode)
        {
            var exceptionData =
                new Dictionary<string, string> { { Constants.ExceptionDataKeyForErrorCode, errorCode } }
                .ToNonGenericDictionary();

            new { exception }.Must().NotBeNull(data: exceptionData);
            exception.Data.Named(Invariant($"{nameof(exception)}.{nameof(Exception.Data)}")).Must().NotBeNull(data: exceptionData);
            new { errorCode }.Must().NotBeNullNorWhiteSpace();
            exception.Data.Keys.OfType<string>().Named(Invariant($"{nameof(exception)}.{nameof(Exception.Data)}.{nameof(IDictionary.Keys)}")).Must().NotContain(Constants.ExceptionDataKeyForErrorCode, data: exceptionData);
            exception.Data.Keys.OfType<string>().Named(Invariant($"{nameof(exception)}.{nameof(Exception.Data)}.{nameof(IDictionary.Keys)}")).Must().NotContain(Constants.ExceptionDataKeyForErrorCodesVector, data: exceptionData);

            exception.Data.Add(Constants.ExceptionDataKeyForErrorCode, errorCode);

            var errorCodesVector = BuildErrorCodesVector(exception);
            exception.Data.Add(Constants.ExceptionDataKeyForErrorCodesVector, errorCodesVector);
            ...
            this Exception exception)
        {
            ...
            var result = GetErrorCodeString(exception, Constants.ExceptionDataKeyForErrorCode);
            ...
            this Exception exception)
        {
            ...
            var result = GetErrorCodeString(exception, Constants.ExceptionDataKeyForErrorCodesVector);
